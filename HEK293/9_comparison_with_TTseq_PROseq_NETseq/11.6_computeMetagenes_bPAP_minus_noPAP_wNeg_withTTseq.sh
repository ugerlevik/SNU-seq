...
## Script purpose: Compute matrices for metagene profiles (bPAP minus noPAP with negatives) and TTseq
## Date: Apr 4, 2025
...
bwFolder2=$prefix/1_Umut/9.1_bPAPminusnoPAP_bigwig/negativesNotRemoved
bwFolder3=$prefix/4_TTseq_Phil/1.5_genebodyNormalised_bigwig

matrixFolder=$prefix/1_Umut/11.1_matrices_bPAPminusnoPAP_wNegatives_withTTseq
...
allbwFiles=("$bwFolder1"/*_3UTRnormalised.bw "$bwFolder2"/*_3UTRnormalised_minus_noPAP_wNeg.bw "$bwFolder3"/*_genebodyNormalised.bw)

# Separate the forward and reverse strand files
fwdbwFiles=()
revbwFiles=()
for file in "${allbwFiles[@]}"; do
  if [[ "$file" == *"fwd_3UTRnormalised.bw" || "$file" == *"fwd_3UTRnormalised_minus_noPAP_wNeg.bw" || "$file" == *"fwd_genebodyNormalised.bw" ]]; then
    fwdbwFiles+=("$file")
  elif [[ "$file" == *"rev_3UTRnormalised.bw" || "$file" == *"rev_3UTRnormalised_minus_noPAP_wNeg.bw" || "$file" == *"rev_genebodyNormalised.bw" ]]; then
    revbwFiles+=("$file")
  fi
done
...
for file in "${fwdbwFiles[@]}"; do
  if [[ "$file" == *"minus_noPAP_wNeg.bw" ]]; then
    sample=$(basename "$file" | sed -E 's/_fwd_3UTRnormalised_minus_noPAP_wNeg.bw/_minus_noPAP_wNeg/')
  elif [[ "$file" == *"fwd_genebodyNormalised.bw" ]]; then
    sample=$(basename "$file" | sed -E 's/_fwd_genebodyNormalised.bw//')
  else
...
for file in "${revbwFiles[@]}"; do
  if [[ "$file" == *"minus_noPAP_wNeg.bw" ]]; then
    sample=$(basename "$file" | sed -E 's/_rev_3UTRnormalised_minus_noPAP_wNeg.bw/_minus_noPAP_wNeg/')
  elif [[ "$file" == *"rev_genebodyNormalised.bw" ]]; then
    sample=$(basename "$file" | sed -E 's/_rev_genebodyNormalised.bw//')
  else
